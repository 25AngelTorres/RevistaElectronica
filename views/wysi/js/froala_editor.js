...
! function(a) {
  ...
  b.count = 0, b.VALID_NODES = ["P", "PRE", "BLOCKQUOTE", "H1", "H2", "H3", "H4", "H5", "H6", "DIV", "LI", "TD"], b.LANGS = [], b.DEFAULTS = {
    ...
    imageUploadToS3: !1,
    imageUploadURL: "http://localhost/RevistaElectronica/views/upload/upload.php",
    imagesLoadURL: "http://localhost/RevistaElectronica/views/upload",
    imagesLoadParams: {},
    inlineMode: !0,
    initOnClick: !1,
    language: "en_us",
    ...
    return a = a.replace(/<b *>(((?!<\/b>)[\s\S])*)<\/b *>/gi, "<strong>$1</strong>"), a = a.replace(/<i>(((?!<\/i>)[\s\S])*)<\/i>/gi, "<em>$1</em>")
    ...
    var b = this.getHTML();
    this.isHTML || (this.trackHTML !== b && null != this.trackHTML ? (this.callback("contentChanged", [], !1), this.refreshImageList(), this.trackHTML = b) : null == this.trackHTML && (this.trackHTML = b)), this.$textarea && this.$textarea.val(b), this.isResizing() || this.isLink || this.isImage || this.options.editInPopup || this.android() || this.imageMode || (this.cleanify(), this.$element.focus())
    ...
    this.browserFixes(), this.isImage || this.isLink || this.options.editInPopup || (this.initUndoRedo(), this.enableTyping(), this.initShortcuts()), this.initEditor(), this.isLink || this.initDrag(), this.initOptions(), this.initEditorSelection(), this.initAjaxSaver(), (!this.isLink || this.isImage) && (this.initImageResizer(), this.initImagePopup()), this.initLink(), this.setLanguage(), this.setCustomText(), this.isImage || this.isLink || this.registerPaste(), this.$element.blur(), this.initialized = !0
    ...
    this.$element.get(0).onpaste = function() {
      if (!b.isHTML) {
        ...
        c.focus(), window.setTimeout(function() {
          var e = c.html();
          c.remove(), a(window).scrollTop(d), b.restoreSelection(), b.insertHTML(b.options.plainPaste ? e.replace(/<(?!br\s*\/?)[^>]+>/g, "") : e.match(/(class=\"?Mso|style=\"[^\"]*\bmso\-|w:WordDocument)/gi) ? b.wordClean(e) : b.clean(e, !1, !0)), b.sync(), b.$element.trigger("placeholderCheck"), b.pasting = !1, b.callback("afterPaste")
        }, 1)
      }
    }
    ...
    return a = this.html5Compliant(a)
    ...
    h.find("*").each(function(b, c) {
      ...
      }), d === !0 && h.removeAttr("style"), "A" == c.tagName && h.attr("href", g.sanitizeURL(h.attr("href"))), "SPAN" == c.tagName && h.attr("data-fr-verified", !0)
    });
    var i = this.cleanNewLine(h.html());
    return this.no_verify = !1, i
    ...
    this.typingTimer = null, this.$element.on("keydown", a.proxy(function() {
      clearTimeout(this.typingTimer), this.ajaxSave = !1, this.oldHTML = this.getHTML(), this.typingTimer = setTimeout(a.proxy(function() {
        this.getHTML().replace(/[\u3041-\u3096]|[\u30A0-\u30FF]|[\u4E00-\u9FFF]/gi, "") != this.oldHTML.replace(/[\u3041-\u3096]|[\u30A0-\u30FF]|[\u4E00-\u9FFF]/gi, "") && ((this.isEnabled("undo") || this.isEnabled("redo")) && this.$element.html() != this.undoStack[this.undoIndex - 1] && this.saveUndoStep(), this.sync())
    ...
    this.$element.find("a").data("fr-link", !0), this.$element.find(".f-img-editor > img").each(function(b, c) {
      a(c).css("padding-left", a(c).parent().css("margin-left")).css("padding-right", a(c).parent().css("margin-right")).css("padding-bottom", a(c).parent().css("margin-bottom")).css("padding-top", a(c).parent().css("margin-top"))
    });
    var c = this.$element.html();
    return this.$element.find(".f-img-editor > img").css("padding", "0px"), c = this.html5Compliant(c), c = this.syncCleanHTML(c, b), c = c.replace(/\s*contenteditable="[^"]*"/gi, ""), c = c.replace(/<a[^>]*?><\/a>/g, ""), void 0 === b && (c = this.removeMarkersByRegex(c)), c = c.replace(/<span[^>]*? class\s*=\s*["']?f-img-handle[^>]+><\/span>/gi, ""), c = c.replace(/^([\S\s]*)<span[^>]*? class\s*=\s*["']?f-img-editor[^>]+>([\S\s]*)<\/span>([\S\s]*)$/gi, "$1$2$3"), c = c.replace(/^([\S\s]*)<span[^>]*? class\s*=\s*["']?f-img-wrap[^>]+>([\S\s]*)<\/span>([\S\s]*)$/gi, "$1$2$3"), c = c.replace(/\&amp;/gi, "&"), this.options.simpleAmpersand && (c = c.replace(/\&amp;/gi, "&")), c.replace(/\u200B/gi, "")
    ...
    if (void 0 == a && (a = !0), !this.isHTML) {
      a && this.$element.focus();
      var c = this.getRange();
      if ("" === this.text() && (0 == c.startOffset || c.startContainer === this.$element.get(0))) {
        ...
        f = this.$element.find(b.VALID_NODES.join(","));
        for (var d = 0; d < f.length; d++)
          if (e = f[d], !this.textEmpty(e)) return void this.setSelection(e);
    ...
    if (this.isHTML || (this.$element.focus(), this.selectionInEditor() || this.setSelection(this.$element.get(0))), this.browser.msie) {
      var c, d;
    ...
    } else this.saveSelectionByMarkers(), document.execCommand("inserthtml", !1, a), this.restoreSelectionByMarkers()
    ...
    a(window).on("hide." + this._id, a.proxy(function() {
      ...
      }!a.ctrlKey && this.$popup_editor.is(":visible") && (this.hide(), this.closeImageMode())
  ...
  a.fn.editable = function(c) {
  ...
  }, $.Editable.prototype._startInDefault = function(a, b) {
  ...
  }, $.Editable.prototype._startInFontExec = function(a, b, c) {
    this.$element.focus(), this.insertHTML('<span data-inserted="true" data-fr-verified="true" data-font="' + b + '" style="' + a + ": " + c + '"></span>', !0);
    ...
    for (var a = 0; a < this.options.buttons.length; a++) {
      ...
      void 0 != $.Editable.commands[b] && (void 0 != $.Editable.commands[b].disabled && $.Editable.commands[b].disabled.call(this) === !0 ? this.$editor.find('[data-name="' + b + '"] button').prop("disabled", !0) : this.$editor.find('[data-name="' + b + '"] button').removeProp("disabled"))
    }
      ...
      for (var c = 0; c < a.length; c++) {
        ...
        if (f.length > 0) {
          ...
          try {
            ...
            g.setEndBefore(f), g.setStartAfter(e), b.addRange(g)
          } catch (h) {}
        }
      }
    ...
    try {
      var d = a.cloneRange();
      d.collapse(b), d.insertNode($('<span class="f-marker" data-fr-verified="true" data-type="' + b + '" data-id="' + c + '">', document)[0]), d.detach()
    } catch (e) {}
    ...
    var a = '<div class="froala-popup froala-image-popup" style="display: none;"><h4><span data-text="true">Insert image</span><i title="Cancel" class="fa fa-times" id="f-image-close-' + this._id + '"></i></h4>';
    ...
    if (this.addListener("hidePopups", $.proxy(function() {
      ...
      this.$image_wrapper.on("change", 'input[type="file"]', function() {
        if (void 0 !== this.files) a.uploadFile(this.files);
          ...
          if (a.options.imageUploadToS3 !== !1) {
    ...
    if (this.options.imageDeleteURL) {
      ...
      b.info = a.data("info"), b.src = a.attr("src"), $.post(this.options.imageDeleteURL, b, $.proxy(function(b) {
    ...
    var a = this,
      b = $('<span data-fr-verified="true">').addClass("f-img-handle").on({
        ...
        moveend: function() {
          $(this).removeAttr("data-start-x"), $(this).removeAttr("data-start-y"); {
            var b = $(this);
            b.prevAll("img")
          }
          a.$element.removeClass("f-non-selectable"), a.isImage || a.$element.attr("contenteditable", !0), a.callback("imageResizeEnd"), $(this).trigger("mouseup")
        }
      });
    return b
    ...
    document.addEventListener && document.addEventListener("drop", $.proxy(function() {
      ...
      if (!a.isResizing()) {
        ...
        if (a.$image_editor.find('button[data-cmd="floatImage' + c.charAt(0).toUpperCase() + c.slice(1) + '"]').addClass("active"), a.$image_editor.find('.f-image-alt input[type="text"]').val($(this).attr("alt") || $(this).attr("title")), a.showImageEditor(), $(this).parent().hasClass("f-img-editor") && "SPAN" == $(this).parent().get(0).tagName) {
          var d = $(this).css("float");
          $(this).parents(".f-img-wrap").css("float", d)
        } else {
          $(this).wrap('<span data-fr-verified="true" class="f-img-editor" style="float: ' + $(this).css("float") + "; margin-left:" + $(this).css("padding-left") + " ; margin-right:" + $(this).css("padding-right") + "; margin-bottom: " + $(this).css("padding-bottom") + "; margin-top: " + $(this).css("padding-bottom") + ';"></span>'), $(this).css("padding-left", "0"), $(this).css("padding-right", "0"), $(this).css("padding-bottom", "0"), $(this).css("padding-top", "0");
          var d = $(this).css("float");
          0 !== $(this).parents(".f-img-wrap").length || a.isImage ? $(this).parents(".f-img-wrap").css("float", d) : $(this).parents("a").length > 0 ? $(this).parents("a:first").wrap('<span data-fr-verified="true" class="f-img-wrap" style="float: ' + d + '"></span>') : $(this).parent().wrap('<span data-fr-verified="true" class="f-img-wrap" style="float: ' + d + '"></span>')
        }
        var e = a.imageHandle();
        $(this).parent().find(".f-img-handle").remove(), a.options.imageResize && ($(this).parent().append(e.clone(!0).addClass("f-h-ne")), $(this).parent().append(e.clone(!0).addClass("f-h-se")), $(this).parent().append(e.clone(!0).addClass("f-h-sw")), $(this).parent().append(e.clone(!0).addClass("f-h-nw"))), a.clearSelection(), a.showByCoordinates($(this).offset().left + $(this).width() / 2, $(this).offset().top + $(this).height()), a.imageMode = !0, a.$bttn_wrapper.find(".fr-bttn").removeClass("active")
      }
    })
    ...
    if ($.Editable.LANGS[this.options.language] && (c = $.Editable.LANGS[this.options.language].translation[c]), confirm(c)) {
      if (this.callback("beforeRemoveImage", [$(b)], !1) === !0) {
        ...
        a.parents(".f-img-wrap").length ? a.parents(".f-img-wrap").remove() : a.remove(), this.refreshImageList(!0), this.hide(), this.saveUndoStep(), this.callback("afterRemoveImage", [$(b)]), this.focus(), this.imageMode = !1
      }
  ...
  }, $.Editable.prototype.addImageWrapper = function() {}, $.Editable.prototype.buildDragUpload = function() {
    var a = this;
    ...
    return c.onerror = $.proxy(function() {
      ...
      var b = this.$element.find(".f-img-editor > img");
      b.attr("src", a), this.hide(), this.hideImageLoader(), this.$image_editor.show(), this.saveUndoStep(), this.callback("replaceImage", [b.get(0)])
    }, this), c.src = a, !1) : (c.onload = $.proxy(function() {
      this.insertLoadedImage(a, b)
    }, this), void(c.src = a))
  }, $.Editable.prototype.insertLoadedImage = function(a) {
    this.$element.focus(), this.restoreSelection(), this.callback("imageLoaded", [a], !1);
    var b = '<img alt="Image title" src="' + a + '" width="' + this.options.defaultImageWidth + '" style="min-width: 16px; min-height: 16px; padding-bottom: ' + this.options.imageMargin + "px; margin-left: auto; margin-right: auto; padding-top: " + this.options.imageMargin + 'px">',
    ...
    }), this.hide(), this.hideImageLoader(), this.saveUndoStep(), this.focus(), this.wrapText(), this.callback("insertImage", [a])
    ...
    if (void 0 !== a && a.length > 0) {
      ...
      if (b) {
        ...
        this.options.crossDomain ? d = this.createCORSRequest("POST", this.options.imageUploadURL) : (d = new XMLHttpRequest, d.open("POST", this.options.imageUploadURL)), d.onload = $.proxy(function() {
          ...
          try {
            this.options.imageUploadToS3 ? 201 == d.status ? this.parseImageResponseXML(d.responseXML) : this.throwImageError(3) : d.status >= 200 && d.status < 300 ? this.parseImageResponse(d.responseText) : this.throwImageError(3)
          } catch (a) {
    ...
    if (!this.isLink && !this.options.editInPopup) {
      ...
      var b = [],
        c = [],
        d = this;
      if (this.$element.find("img").each(function(a, e) {
        var f = $(e);
        if (b.push(f.attr("src")), c.push(f), "right" == f.css("float")) {
          var g = f.parent();
          g.hasClass("f-img-editor") ? (g.css("margin-left", d.options.imageMargin).css("margin-bottom", d.options.imageMargin).css("margin-top", d.options.imageMargin).css("margin-right", "3px"), f.css("margin", "auto")) : f.css("padding-left", d.options.imageMargin).css("padding-bottom", d.options.imageMargin).css("padding-top", d.options.imageMargin).css("padding-right", "3px")
        } else if ("left" == f.css("float")) {
          var g = f.parent();
          g.hasClass("f-img-editor") ? (g.css("margin-right", d.options.imageMargin).css("margin-bottom", d.options.imageMargin).css("margin-top", d.options.imageMargin).css("margin-left", "3px"), f.css("margin", "auto")) : f.css("padding-right", d.options.imageMargin).css("padding-bottom", d.options.imageMargin).css("padding-top", d.options.imageMargin).css("padding-left", "3px")
        }
      }), void 0 === a)
        for (var e = 0; e < this.imageList.length; e++) b.indexOf(this.imageList[e].attr("src")) < 0 && this.callback("afterRemoveImage", [this.imageList[e]], !1);
      this.imageList = c
    }
  }, $.Editable.prototype.showLinkWrapper = function() {
    this.$link_wrapper && (this.$link_wrapper.show(), this.$link_wrapper.trigger("hideLinkList"), this.$link_wrapper.trigger("hideLinkClassList"), setTimeout($.proxy(function() {
    ...
    var a = this,
      ...
      c = function(b) {
        b.stopPropagation(), b.preventDefault(), a.link = !0, a.clearSelection(), a.removeMarkers(), a.selectionDisabled || ($(this).before('<span class="f-marker" data-type="true" data-id="0" data-fr-verified="true"></span>'), $(this).after('<span class="f-marker" data-type="false" data-id="0" data-fr-verified="true"></span>')), a.restoreSelectionByMarkers(), a.exec("createLink");
        var c = $(this).attr("href") || "";
        a.$link_wrapper.find("input.f-lt").val($(this).text()), a.isLink ? ("#" == c && (c = ""), a.$link_wrapper.find("input#f-lu-" + a._id).val(c.replace(/\&amp;/g, "&")), a.$link_wrapper.find(".f-external-link").attr("href", c || "#")) : (a.$link_wrapper.find("input.f-lu").val(c.replace(/\&amp;/g, "&")), a.$link_wrapper.find(".f-external-link").attr("href", c)), a.$link_wrapper.find('input[type="checkbox"]').prop("checked", "_blank" == $(this).attr("target")), a.$link_wrapper.find("li.f-choose-link-class").each($.proxy(function(a, b) {
          $(this).hasClass($(b).data("class")) && $(b).click()
        }, this)), a.showByCoordinates($(this).offset().left + $(this).outerWidth() / 2, $(this).offset().top + (parseInt($(this).css("padding-top")) || 0) + $(this).height()), a.$link_wrapper.find("input.f-lu").focus(), a.closeImageMode(), a.showInsertLink()
      };
  ...
  }, $.Editable.prototype.writeLink = function(a, b, c, d, e) {
    this.options.noFollow && (e = !0), this.options.alwaysBlank && (d = !0);
    var f = "",
      g = "";
    if (e === !0 && /^https?:\/\//.test(a) && (f = 'rel="nofollow"'), d === !0 && (g = 'target="_blank"'), a = this.sanitizeURL(a), this.imageMode) {
      if ("" !== a) {
        if ("A" != this.$element.find(".f-img-editor").parent().get(0).tagName) this.$element.find(".f-img-editor").wrap('<a data-fr-link="true" href="' + a + '" ' + g + " " + f + "></a>");
        else {
          var h = this.$element.find(".f-img-editor").parent();
          d === !0 ? h.attr("target", "_blank") : h.removeAttr("target"), e === !0 ? h.attr("rel", "nofollow") : h.removeAttr("rel"), h.removeClass(Object.keys(this.options.linkClasses).join(" ")), h.attr("href", a).addClass(c)
        }
        this.callback("insertImageLink", [a])
      } else "A" == this.$element.find(".f-img-editor").parent().get(0).tagName && $(this.$element.find(".f-img-editor").get(0)).unwrap(), this.callback("removeImageLink");
      this.saveUndoStep(), this.showImageEditor(), this.$element.find(".f-img-editor").find("img").click(), this.link = !1
    } else {
      if (this.isLink ? "" == b && (b = this.$element.text()) : (this.restoreSelection(), document.execCommand("unlink", !1, a), this.saveSelectionByMarkers(), this.$element.find('span[data-fr-link="true"]').each(function(a, b) {
        $(b).replaceWith($(b).html())
      }), this.restoreSelectionByMarkers()), "" !== a) {
        var i;
        this.isLink ? (this.$element.text(b), i = [this.$element.attr("href", a).get(0)]) : (document.execCommand("createLink", !1, a), i = this.getSelectionLinks());
        for (var j = 0; j < i.length; j++) d === !0 ? $(i[j]).attr("target", "_blank") : $(i[j]).removeAttr("target"), e === !0 && /^https?:\/\//.test(a) ? $(i[j]).attr("rel", "nofollow") : $(i[j]).removeAttr("rel"), $(i[j]).data("fr-link", !0), $(i[j]).removeClass(Object.keys(this.options.linkClasses).join(" ")), $(i[j]).addClass(c);
    ...
    this.addListener("hidePopups", this.hideLinkWrapper), this.isLink && this.options.linkText && this.$link_wrapper.on("mouseup keydown", "input#f-lt-" + this._id, $.proxy(function(a) {
      ...
      a.$link_wrapper.trigger("hideLinkClassList"), $(this).find("i").toggleClass("fa-chevron-down"), $(this).find("i").toggleClass("fa-chevron-up"), c.toggle()
    ...
    })), Object.keys(this.options.linkClasses).length && (this.$link_wrapper.on("mouseup keydown", "input#f-luc-" + this._id, $.proxy(function(a) {
      a.stopPropagation(), this.$link_wrapper.trigger("hideLinkList"), this.$link_wrapper.trigger("hideLinkClassList")
    }, this)), this.$link_wrapper.on("click touch", "li.f-choose-link-class", function() {
      var b = a.$link_wrapper.find("input#f-luc-" + a._id);
      b.val($(this).text()), b.data("class", $(this).data("class")), a.$link_wrapper.trigger("hideLinkClassList")
    }).on("mouseup", "li.f-choose-link-class", function(a) {
      a.stopPropagation()
    }), this.$link_wrapper.on("click", "button#f-links-class-" + this._id, function(b) {
      b.stopPropagation(), a.$link_wrapper.trigger("hideLinkList");
      var c = a.$link_wrapper.find("ul#f-link-class-list-" + a._id);
      ...
      var b = a.$link_wrapper.find("ul#f-link-class-list-" + a._id),
        c = a.$link_wrapper.find("button#f-links-class-" + a._id);
      b && b.is(":visible") && c.click()
    })), this.$link_wrapper.on("click", "i#f-link-close-" + this._id, $.proxy(function() {
      this.$bttn_wrapper.show(), this.hideLinkWrapper(), (!this.options.inlineMode && !this.imageMode && 0 == this.options.buttons.length || this.isLink) && this.hide(), this.imageMode ? this.showImageEditor() : this.restoreSelection()
    }, this))
    ...
    return e
  }, $.Editable.TAG_ORDER = ["table", "thead", "tbody", "tfoot", "tr", "th", "td", "ul", "ol", "li", "h1", "h2", "h3", "h4", "h5", "h6", "pre", "blockquote", "p", "div", "a", "strong", "em", "strike", "u", "span"], $.Editable.SEPARATE = ["th", "td", "li", "h1", "h2", "h3", "h4", "h5", "h6", "pre", "blockquote", "p", "div"], $.Editable.prototype.tagKey = function(a) {
    return a.type + (a.attrs || []).sort().join("|")
  }, $.Editable.prototype.extendedKey = function(a) {
    return this.tagKey(a) + JSON.stringify(a.style)
  }, $.Editable.prototype.mergeStyle = function(a, b) {
    for (var c = {}, d = ["font_size", "font_family", "color", "background_color"], e = 0; e < d.length; e++) {
      var f = d[e];
      c[f] = null != b.style[f] ? b.style[f] : a.style[f]
    }
    return c
  }, $.Editable.prototype.mapDOM = function(a) {
    for (var b, c = 0, d = [], e = "", f = [], g = ["br", "button", "input", "img"], h = {}, i = {}, j = {}, k = 0; k < a.length; k++)
      if (b = a.charAt(k), "<" == b) {
        var l = a.indexOf(">", k + 1);
        if (-1 !== l) {
          var m = a.substring(k, l + 1),
            n = m.replace(/^<\/?([a-z]+)([^<]+)*>$/gi, "$1");
          if ("b" == n && (n = "strong"), "i" == n && (n = "em"), g.indexOf(n) >= 0) {
            j[c] || (j[c] = []), j[c].push(m), k = l;
            continue
          }
          var o = null !== m.match(/^<\/([a-z]+)([^<]+)*>$/gi),
            p = null,
            q = null,
            r = null,
            s = null,
            t = null,
            u = m.replace(/^<[\S\s]* style=("[^"]+"|'[^']+')[\S\s]*>$/gi, "$1");
          if (u != m && (u = u.substring(1, u.length - 1), p = u.replace(/^[\S\s]*font-size: *([^;]+)[\S\s]*$/gi, "$1"), p == u && (p = null), q = u.replace(/^[\S\s]*font-family: *([^;]+)[\S\s]*$/gi, "$1"), q == u && (q = null), r = u.replace(/^[^-]*color: *([^;]+)[\S\s]*$/gi, "$1"), r == u && (r = null), s = u.replace(/^[\S\s]*background-color: *([^;]+)[\S\s]*$/gi, "$1"), s == u && (s = null)), t = m.match(/([\w\-]*)=("[^<>"]*"|'[^<>']*'|\w+)/gi))
            for (var v = 0; v < t.length; v++) 0 == t[v].indexOf("style=") && t.splice(v, 1);
          t && t.indexOf('class="f-marker"') >= 0 && (i[c] || (i[c] = []), i[c].push(m)), o ? h[n].pop() : (d.push({
            type: n,
            attrs: t,
            style: {
              font_size: p,
          ...
          }), h[n] || (h[n] = []), h[n].push(d.length - 1)), k = l
        }
      } else {
        e += b, f[c] = {};
        for (var n in h)
          for (var w = h[n], x = 0; x < w.length; x++) {
            var m = d[w[x]],
              y = this.tagKey(m);
            f[c][y] ? f[c][y].style = this.mergeStyle(f[c][y], m) : f[c][y] = $.extend({}, m)
          }
        c++
      }
    for (var z = [], k = 0; k < f.length; k++) {
      z[k] = {};
      for (var A in f[k]) z[k][this.extendedKey(f[k][A])] = f[k][A]
    }
    return {
      text: e,
      format: z,
  ...
  }, $.Editable.prototype.froalaDOM = function(a) {
    for (var b = [], c = {}, d = 0; d < a.length; d++) {
      var e = a[d];
      for (var f in c) e[f] || (c[f].end = d, b.push($.extend({}, c[f])), delete c[f]);
      for (var g in e) c[g] || (e[g].start = d, c[g] = e[g])
    }
    for (var f in c) c[f].end = a.length, b.push(c[f]);
    return b
  }, $.Editable.prototype.sortNodes = function(a, b) {
    return $.Editable.TAG_ORDER.indexOf(a.type) > $.Editable.TAG_ORDER.indexOf(b.type)
  }, $.Editable.prototype.openTag = function(a) {
    var b = "<" + a.type;
    if (a.attrs)
  ...
  }, $.Editable.prototype.cleanOutput = function(a) {
    for (var b = this.mapDOM(a), c = this.froalaDOM(b.format), d = b.markers, e = b.self_enclosing, f = b.text, g = {}, h = 0; h < c.length; h++) {
      var i = c[h];
      g[i.start] || (g[i.start] = []), g[i.start].push(i)
    }
    var j, k = {},
      a = "";
    for (h = 0; h <= f.length; h++) {
      var l = [];
      if (d[h])
        for (var m = 0; m < d[h].length; m++) a += d[h][m] + "</span>";
      if (e[h])
        for (m = 0; m < e[h].length; m++) a += e[h][m];
      if (k[h]) {
        for (var n in k)
          if (n > h)
            for (j = 0; j < k[n].length; j++) {
              var o = k[n][j];
              o.start >= k[h][k[h].length - 1].start && $.Editable.TAG_ORDER.indexOf(o.type) > $.Editable.TAG_ORDER.indexOf(k[h][k[h].length - 1].type) && $.Editable.SEPARATE.indexOf(o.type) < 0 && (a += "</" + o.type + ">", l.push(o), k[n].splice(j, 1))
            }
        for (j = 0; j < k[h].length; j++) a += "</" + k[h][j].type + ">"
      }
      for (g[h] || (g[h] = []); l.length > 0;) {
        var p = l.pop();
        p.start = h, g[h].push(p)
      }
      if (g[h])
        for (g[h].sort(this.sortNodes), j = 0; j < g[h].length; j++) {
          var i = g[h][j];
          k[i.end] || (k[i.end] = []), k[i.end].push(i), a += this.openTag(i)
        }
      h != f.length && (a += f[h])
    }
    return a
  }, $.Editable.prototype.cleanify = function() {
    this.saveSelectionByMarkers();
    var a = this.getSelectionElements();
    if (a[0] != this.$element.get(0))
      for (var b = 0; b < a.length; b++) {
        var c = $(a[b]);
        c.html(this.cleanOutput(c.html()))
      }
    ...
    this.isImage || this.isLink || this.options.editInPopup || this.$element.on("keydown", $.proxy(function(a) {
      var b = a.which,
        c = ["PRE", "BLOCKQUOTE"],
        d = this.getSelectionElements()[0];
      if (13 == b && "" === this.text() && c.indexOf(d.tagName) >= 0)
        if (this.getSelectionTextInfo(d).atEnd && !a.shiftKey) {
          a.preventDefault();
          var e = $("<p><br></p>");
          $(d).after(e), this.setSelection(e.get(0))
        } else(this.browser.webkit || this.browser.msie) && (a.preventDefault(), this.insertHTML(this.endsWith($(d).html(), "<br>") || !this.getSelectionTextInfo(d).atEnd ? "<br>" : "<br><br>"))
    }, this))
  }, $.Editable.prototype.liEnterSafari = function() {
    this.browser.safari && (this.isImage || this.isLink || this.options.editInPopup || this.$element.on("keyup", $.proxy(function(a) {
      var b = a.which;
      if (13 == b && "" === this.text() && !a.shiftKey) {
        var c = this.getSelectionElement();
        if ($(c).parents("li").length > 0) {
          var d = $(c).parents("li:first");
          this.saveSelectionByMarkers();
          var e = !0;
          d.find($.Editable.VALID_NODES.join(",")).each(function(a, b) {
            e || $(b).before('<span data-fr-verified="true" class="end-li"></span>'), e = !1
          });
          var f = d.html();
          f = f.replace(/<span data-fr-verified=\"true\" class=\"end-li\"><\/span>/g, "</li><li>"), d.replaceWith("<li>" + f + "</li>"), this.restoreSelectionByMarkers()
        }
      }
    }, this)))
  }, $.Editable.prototype.fixBadSpan = function() {
    this.$element.on("DOMNodeInserted", $.proxy(function(a) {
      if ("SPAN" != a.target.tagName || $(a.target).data("fr-verified") || this.no_verify || ($(a.target).before($(a.target).contents()), $(a.target).remove()), "BR" == a.target.tagName) {
        var b = $(a.target).parent();
        "LI" == b.get(0).tagName && b.find($.Editable.VALID_NODES.join(",")).length > 0 && $(a.target).remove()
      }
    }, this))
  },
  function(a) {
